import { getAllDbData, getAllDbDataByIndex, getDbData } from './db';
import { sortData, paginateData } from './dataHelper';

const CONFIG = process.env.CONFIG;

const getAllCategories = async () => {
  let categories = await getAllDbData('categories');
  return sortData(categories, 'priority', 'desc');
};

const getAllPostsByCategory = async (categoryId) => {
  let posts = await getAllDbDataByIndex('posts', 'category', categoryId);
  return sortData(posts, 'priority', 'desc');
};

const getAllPostsByTag = async (tagId) => {
  let posts = await getAllDbDataByIndex('posts', 'tags', tagId);
  return sortData(posts, 'priority', 'desc');
};
...
const getPostByCompositeId = async (compositeId) => {
  const post = await getDbData('posts', compositeId);
  if (!post) return null;

  const categories = await Promise.all(post.categories.map(catId => getDbData('categories', catId)));
  const tags = await Promise.all(post.tags.map(tagId => getDbData('tags', tagId)));
